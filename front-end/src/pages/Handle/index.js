...
export default function Postgresql(){
  ...
  const [y, setY] = useState(0)
    ...
    async function handleFile(){
      try{
        if(name==''){
          alert("Tabela inexitente!")
        }
        const response = await showtab(name)
    ...
    async function handleLines(){
      const response = await image(name)
              ...
              <Form 
                      ...
                      >
                          ...
                          <Form.Item  >
                            ...
                            <option>None</option>{tables.map(t => (<option value={t} >{t}</option>))}
                          </select><br/>
                  ...
                  <p><p><table style={{width:280,marginTop:70}}><tr border="5" color="white"><td><Button className="crud" style={{marginRight:50}} onClick={handleDelete}>Deletar Tabela</Button></td>
                  <td><Map nome={name} setNome={setName} bases={base} setBases={setBase} /></td></tr></table></p></p>
                  </section>
